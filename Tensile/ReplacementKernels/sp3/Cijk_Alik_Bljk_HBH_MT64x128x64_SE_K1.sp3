...
var vgprLocalWriteAddrA=88
var vgprLocalWriteAddrB=90
...
////constant def/////////////
var varlds_pad            = 8
var varlds_Asize_per_wr   = 128+varlds_pad            //each load inst load one 64X2 block.    need contiunous 64X2X2=256    bytes in LDS
var varlds_Asize_per_wave = varlds_Asize_per_wr * 16  //each wave load 8 64X2 block one time.  need contiunous 64X2X8X2=2048 bytes in LDS
...
var varlds_Bsize_per_wr   = 128+varlds_pad            //each load inst load one 64X2  block.    need contiunous 64X2X2=256     bytes in LDS
var varlds_Bsize_per_wave = varlds_Bsize_per_wr * 32  //each wave load seperate 32X64 block.    need contiunous 64X2X16X2=2048 bytes in LDS
...
shader main
  ...
  // why subtract lds_Asize_per_wr - A_lds_size_wr is used as inst_offset in buffer_load_dwordx4 lds:1
  s_lshl_b32       s[sgprScalarGlobalReadOffsetA+0],  s[sgprStridesA+0],    4   // X4 = X2(2 lines) X2(conver to bytes).  each buffer load process 2 lines.
  ...
  v_lshl_add_u32   v[vgprLocalWriteAddrA], v[vgprSerial+1], 4, s[sgprLocalWriteAddrA+0]

  v_lshrrev_b32    v0, 3, v[vgprSerial+1]
  v_mul_lo_u32     v0, varlds_pad, v0 
  v_add_u32        v[vgprLocalWriteAddrA], v0, v[vgprLocalWriteAddrA]
  ...
  // LVCA= 32
  //glvw = 1
  v_lshrrev_b32    v2,     3,  v[vgprSerial+1] // K is 64, byte of element is 2, so divide by (64*2/4) = 32
  ...
  s_lshl_b32       s[sgprScalarGlobalReadOffsetB+0], s[sgprStridesB+0],    4   // X2 = X2(2 lines) X2(conver to bytes).  each buffer load process 2 lines.
  ...
  v_lshl_add_u32   v[vgprLocalWriteAddrB], v[vgprSerial+1], 4, s[sgprLocalWriteAddrB+0]

  v_lshrrev_b32    v0, 3, v[vgprSerial+1]
  v_mul_lo_u32     v0, varlds_pad, v0
  v_add_u32        v[vgprLocalWriteAddrB], v0, v[vgprLocalWriteAddrB]
  ...
  s_mov_b32        s84, varlds_Asize_per_wg
  v_add_u32        v[vgprLocalWriteAddrA+1], v[vgprLocalWriteAddrA+0], s84
  ...
  buffer_load_dwordx4 v[vgprG2LA+0+4],  v[vgprGlobalReadOfvarA+1],  s[sgprSrdA:sgprSrdA+3], 0 offen:1 offset:0
  ...
  buffer_load_dwordx4 v[vgprG2LB+ 0+ 8],  v[vgprGlobalReadOfvarB+2],  s[sgprSrdB:sgprSrdB+3], 0 offen:1 offset:0
  buffer_load_dwordx4 v[vgprG2LB+ 0+12],  v[vgprGlobalReadOfvarB+3],  s[sgprSrdB:sgprSrdB+3], 0 offen:1 offset:0

  /* increment 64 elements to fetch next k=64 elements of tile 64x64 */
  s_add_u32        s[sgprSrdA+0], s[sgprSrdA+0], 128                         // pad by 4
  s_addc_u32       s[sgprSrdA+1], s[sgprSrdA+1], 0                           // pad by 4

  s_add_u32        s[sgprSrdB+0], s[sgprSrdB+0], 128                         // pad by 4
  s_addc_u32       s[sgprSrdB+1], s[sgprSrdB+1], 0                           // pad by 4

  /* load A buffer to LDS A[1] */
  //Fetch 2nd unroll loop iteration (2nd 64 k indices)
  buffer_load_dwordx4 v[vgprG2LA+8+0],  v[vgprGlobalReadOfvarA+0],  s[sgprSrdA:sgprSrdA+3], 0 offen:1 offset:0
  buffer_load_dwordx4 v[vgprG2LA+8+4],  v[vgprGlobalReadOfvarA+1],  s[sgprSrdA:sgprSrdA+3], 0 offen:1 offset:0


  /* load B buffer to LDS B[1] */
  buffer_load_dwordx4 v[vgprG2LB+16+ 0],  v[vgprGlobalReadOfvarB+0],  s[sgprSrdB:sgprSrdB+3], 0 offen:1 offset:0
  buffer_load_dwordx4 v[vgprG2LB+16+ 4],  v[vgprGlobalReadOfvarB+1],  s[sgprSrdB:sgprSrdB+3], 0 offen:1 offset:0
  buffer_load_dwordx4 v[vgprG2LB+16+ 8],  v[vgprGlobalReadOfvarB+2],  s[sgprSrdB:sgprSrdB+3], 0 offen:1 offset:0
  buffer_load_dwordx4 v[vgprG2LB+16+12],  v[vgprGlobalReadOfvarB+3],  s[sgprSrdB:sgprSrdB+3], 0 offen:1 offset:0
  ...
  s_waitcnt vmcnt(10)
  ds_write_b128      v[vgprLocalWriteAddrA+0],  v[vgprG2LA+0+0]  offset:0
  ds_write_b128      v[vgprLocalWriteAddrA+0],  v[vgprG2LA+0+4]  offset:varlds_Asize_per_wr * 8

  s_waitcnt vmcnt(6)
  ds_write_b128      v[vgprLocalWriteAddrB+0],  v[vgprG2LB+ 0+ 0]  offset:0
  ds_write_b128      v[vgprLocalWriteAddrB+0],  v[vgprG2LB+ 0+ 4]  offset:varlds_Bsize_per_wr *  8
  ds_write_b128      v[vgprLocalWriteAddrB+0],  v[vgprG2LB+ 0+ 8]  offset:varlds_Bsize_per_wr * 16
  ds_write_b128      v[vgprLocalWriteAddrB+0],  v[vgprG2LB+ 0+12]  offset:varlds_Bsize_per_wr * 24
...
label_0005:
  ...
  s_waitcnt vmcnt(10)
  ds_write_b128      v[vgprLocalWriteAddrA+1],  v[vgprG2LA+8+0]  offset:0
  ds_write_b128      v[vgprLocalWriteAddrA+1],  v[vgprG2LA+8+4]  offset:varlds_Asize_per_wr * 8

  s_waitcnt vmcnt(6)
  ds_write_b128      v[vgprLocalWriteAddrB+1],  v[vgprG2LB+16+ 0]  offset:0
  ds_write_b128      v[vgprLocalWriteAddrB+1],  v[vgprG2LB+16+ 4]  offset:varlds_Bsize_per_wr *  8
  ds_write_b128      v[vgprLocalWriteAddrB+1],  v[vgprG2LB+16+ 8]  offset:varlds_Bsize_per_wr * 16
  ds_write_b128      v[vgprLocalWriteAddrB+1],  v[vgprG2LB+16+12]  offset:varlds_Bsize_per_wr * 24
...
wave0_entry_start:
  ...
  v_lshrrev_b32    v2, 0, v1                        // 2 column of A is 256 byte, pad per 256 bytes => divice by 2
...
/*****************************************************************/
  ...
  ds_read_b64       v[vgprValuA_X0_I0+16+ 0:vgprValuA_X0_I0+16+ 1],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32
  ds_read_b64       v[vgprValuA_X0_I0+ 0+ 2:vgprValuA_X0_I0+ 0+ 3],  v[vgprLocalReadAddrA+0]  offset:16
  ds_read_b64       v[vgprValuA_X0_I0+16+ 2:vgprValuA_X0_I0+16+ 3],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 16
  ds_read_b64       v[vgprValuA_X0_I0+ 0+ 4:vgprValuA_X0_I0+ 0+ 5],  v[vgprLocalReadAddrA+0]  offset:32
  ds_read_b64       v[vgprValuA_X0_I0+16+ 4:vgprValuA_X0_I0+16+ 5],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 32
  ds_read_b64       v[vgprValuA_X0_I0+ 0+ 6:vgprValuA_X0_I0+ 0+ 7],  v[vgprLocalReadAddrA+0]  offset:48
  ds_read_b64       v[vgprValuA_X0_I0+16+ 6:vgprValuA_X0_I0+16+ 7],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 48
  ds_read_b64       v[vgprValuA_X0_I0+ 0+ 8:vgprValuA_X0_I0+ 0+ 9],  v[vgprLocalReadAddrA+0]  offset:64
  ds_read_b64       v[vgprValuA_X0_I0+16+ 8:vgprValuA_X0_I0+16+ 9],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 64
  ds_read_b64       v[vgprValuA_X0_I0+ 0+10:vgprValuA_X0_I0+ 0+11],  v[vgprLocalReadAddrA+0]  offset:80
  ds_read_b64       v[vgprValuA_X0_I0+16+10:vgprValuA_X0_I0+16+11],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 80
  ds_read_b64       v[vgprValuA_X0_I0+ 0+12:vgprValuA_X0_I0+ 0+13],  v[vgprLocalReadAddrA+0]  offset:96
  ds_read_b64       v[vgprValuA_X0_I0+16+12:vgprValuA_X0_I0+16+13],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 96
  ds_read_b64       v[vgprValuA_X0_I0+ 0+14:vgprValuA_X0_I0+ 0+15],  v[vgprLocalReadAddrA+0]  offset:112
  ds_read_b64       v[vgprValuA_X0_I0+16+14:vgprValuA_X0_I0+16+15],  v[vgprLocalReadAddrA+0]  offset:varlds_Asize_per_wr * 32 + 112
...
label_0001:
  ...
  for var p =0; p < 2; p++
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+ 0), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+ 2), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 16
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+ 0+ 4), v_regs(vgprLocalReadAddrA,k) offset:32
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+ 4), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 32
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+ 6), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 48
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+ 8), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 64
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+ 0+10), v_regs(vgprLocalReadAddrA,k) offset:80
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+10), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 80
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+12), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 96
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+ 0+14), v_regs(vgprLocalReadAddrA,k) offset:112
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32*k+16+14), v_regs(vgprLocalReadAddrA,k) offset:varlds_Asize_per_wr * 32 + 112
...
/*****************************************************************/
  ...
  //unrolling double
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+ 0), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+ 2), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 16
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+ 0+ 4), v_regs(vgprLocalReadAddrA,1)  offset:32
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+ 4), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 32
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+ 6), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 48
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+ 8), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 64
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+ 0+10), v_regs(vgprLocalReadAddrA,1)  offset:80
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+10), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 80
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+12), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 96
     ...
     ds_read_b64       v_regs(vgprValuA_X0_I0, 32+16+14), v_regs(vgprLocalReadAddrA,1)  offset:varlds_Asize_per_wr * 32 + 112
